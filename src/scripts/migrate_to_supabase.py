...
from src.database.models import UserTable, WorldBibleTable, StoryTable, ChapterTable
...
async def migrate():
    ...
    if not supabase_url or "sqlite" in supabase_url:
        ...
        print("Example: postgresql+asyncpg://postgres:[password]@db.[project-id].supabase.co:5432/postgres")
        return
    ...
    async with SqliteSession() as src_session, SupabaseSession() as dest_session:
        for name, model in models:
            ...
            query_cols = [getattr(model, col_name) for col_name in existing_cols if hasattr(model, col_name)]
