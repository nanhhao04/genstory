...
async def text_to_speech(
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            try:
                response = await client.post(tts_api_url, data=req.text.encode("utf-8"), headers=headers, timeout=30)
                if response.status_code != 200:
                    logging.error("FPT.AI POST Error: %s - %s", response.status_code, response.text)
                    raise HTTPException(status_code=response.status_code, detail="Loi gui yeu cau den FPT.AI")

                payload = response.json()
                audio_url = payload.get("async")
                if not audio_url:
                    raise HTTPException(status_code=500, detail="Khong tim thay URL am thanh tu FPT.AI")
                ...
                for _ in range(30):
                    ...
                    raise HTTPException(status_code=audio_response.status_code, detail="Loi tai file am thanh")

                if not audio_content:
                    raise HTTPException(status_code=408, detail="FPT.AI xu ly qua lau, vui long thu lai sau giay lat.")

                return StreamingResponse(io.BytesIO(audio_content), media_type="audio/mpeg")
            except httpx.TimeoutException as exc:
                raise HTTPException(status_code=408, detail="Yeu cau den FPT.AI bi qua han") from exc
            except HTTPException:
                raise
            except Exception as exc:
                logging.error("TTS Unhandled Error: %s", exc)
                raise HTTPException(status_code=500, detail=f"Loi he thong TTS: {str(exc)}") from exc
    except Exception as exc:
        status = "failed"
        raise exc
    finally:
