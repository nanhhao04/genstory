    ...
    async def generate_content_async(self, prompt, **kwargs):
        ...
        for provider in providers_to_try:
            if provider == "openai":
                ...
                try:
                    print(
                        f"  [LLM] 🚀 Attempting generation with OpenAI (Model: {os.getenv('OPENAI_MODEL', 'gpt-4o-mini')})..."
                    )
            ...
            if provider == "gemini":
                ...
                while attempts < max_attempts:
                    ...
                    try:
                        print(
                            f"  [LLM] 🚀 Attempting Gemini (Key Index: {self.current_gemini_key_idx})..."
                        )
                        # New SDK async call
                        ...
                        print(
                            f"  [LLM] ✅ Gemini success (Key Index: {self.current_gemini_key_idx})."
                        )
                        return response
                    except Exception as e:
                        duration = time.time() - start_time
                        ...
                        # Rotate to next key
                        self.current_gemini_key_idx = (self.current_gemini_key_idx + 1) % len(
                            self.gemini_keys
                        )
                        self._configure_gemini()

                        if attempts < max_attempts:
                            print("  [LLM] 🔄 Retrying with next Gemini key...")
                        else:
                            print("  [LLM] ⚠️ All Gemini keys exhausted.")
...
def connect_llm():
    ...
    hf_model = os.getenv("HF_IMAGE_MODEL") or cfg.get(
